...
export default function PatientHistoryPage() {
  ...
  useEffect(() => {
    api
      .get(`/admin/patients/${patientId}/history`)
      .then((res) => setData(res.data))
      .catch((err) => setError(err?.response?.data?.detail || "Unable to load patient history"));
  }, [patientId]);
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-extrabold">Patient Full History</h1>
            <p className="text-slate-600">All previous medical and billing records for this patient.</p>
            {data?.patient ? <p className="text-sm text-cyan-700 mt-2">{data.patient.full_name} (MRN: {data.patient.patient_mrn})</p> : null}

...
import { BedDouble, Building2, KeyRound, LogOut } from "lucide-react";
import { clearSession, getSessionUser } from "../../lib/auth";
import { useHospitalSettings } from "../../context/HospitalSettingsContext";

const links = [
  { to: "/dashboard/operations", label: "Unit Config", icon: Building2 },
  { to: "/dashboard/account", label: "Account", icon: KeyRound },
];

export default function OperationsWorkspaceLayout({ title, subtitle, children }) {
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="flex min-h-screen">
        <aside className="hidden w-72 bg-slate-900 p-6 text-white md:flex md:flex-col">
          <Link to="/dashboard/operations" className="mb-8 block">
            <p className="text-xs uppercase tracking-[0.2em] text-teal-300">MedX Operations</p>
            <p className="mt-1 text-2xl font-extrabold">Unit Control</p>
          </Link>
            ...
            {links.map((item) => {
              ...
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                  end={item.to === "/dashboard/operations"}
                  className={({ isActive }) =>
        ...
        <main className="flex-1">
          ...
          <section className="p-5 sm:p-8">
            <div className="mb-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">
              <BedDouble className="h-4 w-4 text-teal-700" />
              Real-Time Bed & Unit Sync
            </div>
            {children}
          </section>
        </main>
